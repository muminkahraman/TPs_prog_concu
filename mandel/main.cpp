...
int draw_screen_thread() {
	...
	int n = 0;
	int i;
	std::thread tab_threads[number_of_threads];
	for (i=0; i<number_of_threads; i++) {
		tab_threads[i] =std::thread(draw_screen_worker, std::ref(vals[i]));
	}
	for (i=0; i<number_of_threads; i++) {
		tab_threads[i].join();
		n += vals[i];
	...
	while(1) {
		...
		if (use_threads) {
			...
			if (n != number_of_slices) {
				...
				std::cout << "OK: Right number of slices computed: "
					  << n << " (number " << number_of_slices << ")"
					  << std::endl;
