...
double fct(int num, int nbtours) {
  ...
  fprintf(stdout," %d lancement de la fonction pour %d itérations \n",num , nbtours);
  // #####################################
  // Partie calcul
...
double fct_thread (int i_b, int nbtours_b, double & temps_total_b){
  double temps_fonction = fct(i_b, nbtours_b);
    temps_total_b += temps_fonction;
    return 0;
}
...
int main(int argc, char **argv) {
  ...
  cout << "Th principal : lancement de " << nbthreads << " fois la fonction"
       << endl;
  double vals[nbthreads];
  thread tab_threads[nbthreads];
  double temps_total = 0;
  for (i=0; i<nbthreads; i++) {
    tab_threads[i] =thread(fct_thread, i, nbtours,std::ref(vals[i]) );
  }
  for (i=0; i<nbthreads; i++) {
    tab_threads[i].join();
    temps_total += vals[i];
